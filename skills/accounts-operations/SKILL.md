...
`method` is the provider-side HTTP verb (`GET`, `POST`, `PUT`, `PATCH`, or `DELETE`). `path` must be provider-relative, start with `/`, and never contain `..`. `query` and `body` may never contain secret-shaped keys (`access_token`, `api_key`, `authorization`, `secret`, and similar; rejected with `400`). The connection must be `status: connected`, or this returns `409`.
...
|---|---|
| Shopify | `{api_version}` |
| Meta Ads | `{account_id}` (the `act_<id>` ad-account id) |
...
2. List accounts when the account id is unknown with `GET /accounts`.
3. For account or connection management, use the tables above directly.
4. For querying a connected platform, call `GET /accounts/{account_id}/connections` for the connection id, read the matching `references/{provider}.md`, then run the smallest `POST /connections/{connection_id}` operation that answers the question.
