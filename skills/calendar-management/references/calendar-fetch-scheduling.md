...
For calendar review, fetch the exact target date across every connected calendar. Use Memory to interpret account roles and source quirks. Exclude canceled events and events the user declined. Deduplicate mirrored events by canonical identity and stable event details.
...
Create an all-day calendar event for `remember X on Friday` and a timed event when a time is supplied. `Do X on Friday` belongs in the canonical Task system instead.
