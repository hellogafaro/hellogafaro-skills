...
- Before finalizing a task body, review every available relevant source that can add material execution context. The finished task should contain the useful context we actually have, so the assignee can start without needing a separate handoff.
...
- If the available source review finds no more material context, do not pad the task or mark it weak. A concise task with all known valuable context is complete enough to start.
